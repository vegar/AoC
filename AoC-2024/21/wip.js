...
const keysPressed = (pad, code, robot, cache) => {
  let r = keysPressed1(pad, code, robot, cache);
  //console.log(`${" ".repeat(2 - robot)}${code} -> ${r}`);
  return r;
};
const keysPressed1 = (pad, code, robot, cache) => {
  ...
  for (let i = 0; i < code.length; i++) {
    ...
    if (robot == 0) {
      const min = moves[0].length;
      length += min;
    } else {
      const options = moves.map((m) => {
        const y = keysPressed(ARROW, m, robot - 1, cache);
        return y;
      });

      const min = options.sort((a, b) => a - b)[0];
      length += min;
    }
...
let result = input.reduce((sum, code) => {
  const l = keysPressed(NUMERIC, code, 25, new Map());
