...
const playCreature = (map, creatures, creature, log = false) => {
  ...
  // const paths = options
  //   .map((option) => {
  //     let [steps, path] = findPath(creature, option, map, creatures);
  //     return { steps, path, option };
  //   })
  //   .sort((a, b) =>
  //     a.steps == b.steps
  //       ? a.option.y == b.option.y
  //         ? a.option.x - b.option.x
  //         : a.option.y - b.option.y
  //       : a.steps - b.steps
  //   );
  // if (log) console.log(`  Paths: `);
  // if (log)
  //   paths.forEach((p) =>
  //     console.log(
  //       `   ${p.steps}: ${p.path.map((p) => `${p.y}:${p.x}`).join("->")}`
  //     )
  //   );
  // if (paths.length == 0) {
  //   if (log) console.log(`  no options. Passing.`);
  //   return;
  // }
  //let chosenPath = paths[0];
  //if (chosenPath.steps == Infinity) return;
  //if (log) console.log(`  going for`, chosenPath);
  //if (chosenPath.steps > 0) {
  //  moveCreature(creatures, creature, chosenPath.path[0]);
  //}
...
const playRound = (map, creatures, round, log = false) => {
...
while (true) {
  try {
    creatures = playRound(input, creatures, x, (log = false));
  } catch (e) {
    console.log({ e });
